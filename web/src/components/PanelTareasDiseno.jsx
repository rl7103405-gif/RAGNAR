...
export default function PanelTareasDiseno() {
  ...
  const claveVersiones = useMemo(
    () => [...new Set(encargos.map((e) => e.planVersionId).filter(Boolean))].sort().join(','),
    [encargos]
  )
    ...
    <div>
      ...
      {vista !== 'equipo' && (
        <ListaEncargos
          ...
          ocupado={trabajando}
          onAsignar={(encargo, ot, destinataria, fechaObjetivo) =>
            correr(
              () => asignarOt({ encargo, ot, destinataria, fechaObjetivo, usuario, esPrueba, existentes: asignaciones }),
...
function ListaEncargos({ encargos, avances, asignaciones, equipo, equipoPorJefa, esAdmin, puedeRepartir, ocupado, onAsignar, onCambiar, onCerrarEncargo }) {
  if (!encargos.length) {
    return (
    ...
    return (
      <details key={e.id} className={`tarjeta td-encargo ${cerrado ? 'td-cerrado' : ''}`} open={!cerrado}>
        <summary className="tp-fila" style={{ justifyContent: 'space-between', cursor: 'pointer', flexWrap: 'wrap', gap: 8 }}>
          <div>
            <strong style={{ fontSize: 16 }}>OC {e.oc}</strong>
...
function FilaOt({ fila, encargo, equipo, equipoPorJefa, puedeRepartir, ocupado, onAsignar, onCambiar }) {
            ...
            <button
              ...
              onClick={() => {
                if (a) {
                  ...
                  onAsignar(encargo, fila.ot, { uid: persona.id, nombre: persona.nombreCompleto || '' }, aFecha(fechaObj))
                }
                setDest('')
              }}
            >
    ...
    <details style={{ marginTop: 4 }} onToggle={alAbrir} open={abierto}>
      ...
      {renglones && renglones.length > 0 && (
        <ul style={{ fontSize: 12, paddingLeft: 16, margin: '4px 0' }}>
          {renglones.map((h) => (
            <li key={h.id}>
              rev {h.revision} · {h.antes?.asignadoANombre || '—'}
